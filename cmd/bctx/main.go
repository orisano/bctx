...
func run() error {
	...
	if err != nil {
		return fmt.Errorf("read ignore(path=%v): %w", ignore, err)
	}

	w, outputPath, err := writer(flags.Destination)
	if err != nil {
		return fmt.Errorf("prepare writer: %w", err)
	}
	defer w.Close()
	...
	if outputPath != "" {
		...
		if err != nil {
			return fmt.Errorf("resolve rel path(src=%v, target=%v): %w", flags.Source, outputPath, err)
		}
	...
	if err != nil {
		return fmt.Errorf("validate directory (path=%v): %w", flags.Source, err)
	}
	...
	if flags.Dockerfile != "" {
		...
		if err != nil {
			return fmt.Errorf("read Dockerfile (path=%v): %w", flags.Dockerfile, err)
		}
		name, err := ioutil.TempDir(flags.Source, ".")
		if err != nil {
			return fmt.Errorf("create temporary dir: %w", err)
		}
		...
		if err := ioutil.WriteFile(tempDockerfile, d, 0666); err != nil {
			return fmt.Errorf("write temporary Dockerfile (path=%v): %w", tempDockerfile, err)
		}
...
func writer(dest string) (io.WriteCloser, string, error) {
	...
	case strings.HasPrefix(dest, "gs://"), strings.HasPrefix(dest, "s3://"), strings.HasPrefix(dest, "file://"):
		...
		if err != nil {
			return nil, "", fmt.Errorf("parse destination(dest=%v): %w", dest, err)
		}
		switch u.Scheme {
		case "gs":
		...
		case "file":
			...
			f, err := os.Create(p)
			if err != nil {
				return nil, "", fmt.Errorf("create file(path=%v): %w", p, err)
			}
			return f, p, nil
		}
		panic("unreachable")
	default:
		...
		if err != nil {
			return nil, "", fmt.Errorf("create file(dest=%v): %w", dest, err)
		}
		return f, dest, nil
	}
}
